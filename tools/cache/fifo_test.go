...
// func Pop1(q Queue) interface{} {
// 	q.Pop(func (item interface{}) error{
// 		testFifo := item.(testFifoObject)
// 		testFifo.name = "tranformed name"
// 		return nil
// 	})
// 	return nil
// }
...
func TestFIFO_PopShouldUnblockWhenClosed(t *testing.T) {
	...
	for i := 0; i < jobs; i++ {
		go func() {
			...
			fmt.Println(" input empty structure")
			c <- struct{}{}
		}()
	}
	...
	for i := 0; i < jobs; i++ {
		select {
		case <-c:
			fmt.Println(" output empty structure " , j)
			j++
