...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	...
	if err := func() error {
		...
		select {
		case <-stopCh:
			fmt.Println("=======stopCh rev an item in list function ===========")
			return nil
		case r := <-panicCh:
			panic(r)
		case <-listCh:
	...
	go func() {
		...
		for {
			...
			case <-resyncCh:
			case <-stopCh:
				fmt.Println("=======stopCh rev an item in resync and clean up routine===========")
				return
			case <-cancelCh:
				return
			}
	...
	for {
		...
		select {
		case <-stopCh:
			fmt.Println("=======rev item from stopCh will return nil from the for loop===========")
			return nil
		default:
		}
		...
		if err := r.watchHandler(start, w, &resourceVersion, resyncerrc, stopCh); err != nil {
			...
			fmt.Println("=======watchHandler return an error and exist the handler===========")
			return nil
		}
	}
}
...
func (r *Reflector) watchHandler(start time.Time, w watch.Interface, resourceVersion *string, errc chan error, stopCh <-chan struct{}) error {
	...
	//defer w.Stop()
	defer func(){
		fmt.Println("======will call watcher stop")
		w.Stop()
	}()
...
loop:
	for {
		select {
		case <-stopCh:
			fmt.Println("=======stopCh rev an item; will exit the watchHandler===========")
		...
		case event, ok := <-w.ResultChan():
			...
			switch event.Type {
			case watch.Added:
				fmt.Println("=======watch find event type add; will add into the store===========")
